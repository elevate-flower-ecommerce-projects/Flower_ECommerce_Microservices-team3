...
namespace Address___Store_Coverage_Service.Persistence
{
	...
	{
		private readonly FlowersAddressStoreCoverageDbContext _dbContext;
		private readonly Stack<string> _savepoints = new();
		private int _depth;
		private IDbContextTransaction? _transaction;
		public UnitOfWork(FlowersAddressStoreCoverageDbContext dbContext)
		{
		...
		private async Task<T> ExecuteInternalAsync<T>(Func<Task<T>> action, CancellationToken cancellationToken)
		{
			if (_depth > 0)
			{
				return await ExecuteCoreAsync(action, cancellationToken);
			}

			var strategy = _dbContext.Database.CreateExecutionStrategy();
			return await strategy.ExecuteAsync(() => ExecuteCoreAsync(action, cancellationToken));
		}

		private async Task<T> ExecuteCoreAsync<T>(Func<Task<T>> action, CancellationToken cancellationToken)
		{
